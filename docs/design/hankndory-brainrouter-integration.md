...
- **Fresh-install reviewer contract (crit #2, supersedes the "default reviewer usable on any bare install" AC).** With Manifest enabled (strix), `auto`→cloud works. With **neither** cloud enabled **nor** a measured `local_model_budget_mb[fallback_model]`, a review returns `blocked:budget_unavailable` with a message: "configure `review_admission.local_model_budget_mb` for <model> or enable cloud." This is the **safe** fresh-install behavior (no OOM guess). The installer writes a **commented** `review_admission:` stub into `brainrouter.yaml` so operators see where to calibrate. AC updated accordingly.
