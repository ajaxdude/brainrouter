...
- **Workflow state:** first release = `approved-for-implementation` (human approval **delegated** by the user — "work autonomously and make good decisions"; 2026-09-17); hardening track = `ready-for-human-review` pending the three product decisions. **Implementation is on-branch only; production deploy to strix and any live `~/.omp` mutation are intentionally withheld for the user's return.**
