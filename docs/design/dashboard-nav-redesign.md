...
- **Workflow state:** readiness-blocked (design only; **no implementation** — the user said "discuss, not code yet"). A v2 revision resolves the round-1 Dory findings below; a fresh readiness pass is still owed before `ready-for-human-review`. Human approval pending (user unavailable).
...
- **R-Posture** A single `renderPosture()` + `renderHealthPill()` update contract aggregates the **four existing sources** (routing profile via `/api/routing-profile`; nudge via `/api/nudge`; review+PR via `/api/review/enabled`+`/api/review/pr-guidelines`; health via `refreshHealth`/`/api/service-health`, `server.rs:1819-1915`) — called after each source refreshes, with explicit loading/unavailable/stale states and defined green/amber/red health semantics (e.g. disabled Manifest and idle llama.cpp are not "red"). No new endpoint. *AC:* posture never shows a permanently stale value after any one source updates.
- **R-A11y** Relocating levers into Config is acceptable only with: visible focus, keyboard-operable sub-navigation, `aria-current`/tab semantics on Models sub-tabs, `role="switch"`+`aria-checked` (or native checkbox) on toggles, **text (not color-only)** posture indicators, and links from each posture entry to its Config anchor. *AC:* keyboard-only walkthrough reaches every moved control; posture is legible without color.
...
**Phase 1** is a restructure of `main_dashboard.html`: introduce a compact top "control + posture" strip (reusing the existing `/api/routing-profile`, `/api/review/enabled`, `/api/review/pr-guidelines`, `/api/nudge`, health data already fetched in `refreshSlow`), move the set-and-forget control blocks into the existing **Config** view under a new "Quality" group, relocate Bridges + toolbox-upgrade + the three rare restarts into Config, remove the "All toolboxes" sidebar list, and move the HankNDory ledger panel into the Stream/Overview view. Controls keep their ids/handlers so the JS wiring is unchanged; only their DOM parent moves. The four model-ish nav entries become one "Models" destination with sub-tabs (Activity/Catalog/Downloads/Serving) — the underlying views already exist (`models`, `toolbox-models`, `server-mode`, `/models`); this is a grouping of existing view-switch targets.
...
flowchart TB
  ...
  subgraph Nav["Primary nav — 4 destinations"]
    ...
    MD[Models\nActivity · Catalog · Downloads · Serving]
    BM[Benchmarks]
    CFG[Config]
  end
...
Phase 1 (UI) acceptance is **objective, not just a manual walkthrough**:
- **Structural assertions** (extend `scripts/check-html-js.sh` or a sibling script): every required `id` occurs exactly once; all four destinations and every Models sub-tab target exist; no duplicate/missing ids after the moves; well-formed HTML nesting. Note `check-html-js` today is JS-syntax-only and silently passes when Node/Bun is absent (`:21-25`) — the gate must fail loudly if the runtime is missing.
- **Deterministic control walkthrough**: a scripted check that each moved action still resolves to its original handler/endpoint, and that the Downloads polling gate fires under the new active-state rule.
- **Accessibility/viewport**: keyboard-only reachability of every moved control; posture legible without color; narrow-viewport behavior of the command-bar strip.
- **Regression guard**: the existing Rust suite must stay green (Phase 1 changes no Rust).
...
- **Round 2 — pending** (fresh isolated readiness pass on v2 before `ready-for-human-review`).
