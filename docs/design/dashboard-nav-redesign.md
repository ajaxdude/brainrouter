...
- **Workflow state:** readiness-blocked → v3 resolves both round-2 blockers; a round-3 readiness pass is recommended before `ready-for-human-review`. **No implementation** proceeds regardless (user said "discuss, not code yet"; approval pending).
...
- **Round 3 — pending** (fresh readiness pass on v3).
