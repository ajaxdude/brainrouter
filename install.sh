...
ENV
        ...
        # Ensure PORT is pinned even when docker-compose.yml already existed
        # (e.g. re-runs, or a manually-placed compose file). The compose file
        # defaults to PORT=2099; without this the container binds the wrong port.
        if [[ -f "${MANIFEST_DIR}/.env" ]]; then
            if ! grep -qE '^PORT=' "${MANIFEST_DIR}/.env"; then
                if grep -qE '^#\s*PORT=' "${MANIFEST_DIR}/.env"; then
                    sed -i 's|^#\s*PORT=.*|PORT='"${MANIFEST_PORT}"'|' "${MANIFEST_DIR}/.env"
                else
                    printf '\nPORT=%s\n' "${MANIFEST_PORT}" >> "${MANIFEST_DIR}/.env"
                fi
            fi
            if ! grep -qE '^BETTER_AUTH_URL=' "${MANIFEST_DIR}/.env"; then
                if grep -qE '^#\s*BETTER_AUTH_URL=' "${MANIFEST_DIR}/.env"; then
                    sed -i 's|^#\s*BETTER_AUTH_URL=.*|BETTER_AUTH_URL=http://localhost:'"${MANIFEST_PORT}"'|' "${MANIFEST_DIR}/.env"
                else
