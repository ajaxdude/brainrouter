#!/usr/bin/env bash
set -euo pipefail
claude mcp add-json brainrouter '{"type":"stdio","command":"/home/papa/ai/projects/brainrouter/target/release/brainrouter","args":["mcp","--socket","/run/user/1000/brainrouter.sock"]}' --scope user
if [ "${1:-}" = "--shell-rc" ]; then
  RC="${HOME}/.zshrc"
